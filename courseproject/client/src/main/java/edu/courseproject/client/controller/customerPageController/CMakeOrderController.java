...
public class CMakeOrderController implements Initializable {
    ...
    @FXML
    private Label timeCreditLAbel;

    @FXML
    private ChoiceBox<String> CreditChoiceBox;

    @FXML
    private Label SumCreditLabel;

    @FXML
    private TextField sumCreditField;

    @FXML
    private Button countCreditBut;
    ...
    @FXML
    void countCredit(ActionEvent event) {
        DecimalFormatSymbols s = new DecimalFormatSymbols();
        s.setDecimalSeparator('.');

        DecimalFormat f = new DecimalFormat("#.##", s);
        String creditString = sumCreditField.getText();
        Integer credit = Integer.parseInt(creditString);
        if(credit>fullCostOrder){
            postLabel.setText("Вы не можете взять кредит больше общей суммы заказа - " + String.format("%.3f", fullCostOrder) );
            postLabel.setVisible(true);
        }else {
            String timeString = CreditChoiceBox.getValue();
            String newTime = timeString.replaceAll(" мес.", "");
            Integer timeForCredit = Integer.parseInt(newTime);
            double time = (double)timeForCredit;
            double percent = (0.0125*Math.pow(1.0125, time))/(Math.pow(1.0125, time)-1) * credit;

            String fr = String.format("%.2f", percent);
            postLabel.setText("Кредит оформлен успешно на " + timeString + "\nЕжемесячный платеж - " +  String.format("%.3f", percent));
            postLabel.setVisible(true);
    ...
    void sendOrder(ActionEvent event) {
        ...
        if (deliveryDataField.getValue() == null || ((Integer.parseInt(currentData[0])<Integer.parseInt(delivery[0]))&&(Integer.parseInt(currentData[1])>Integer.parseInt(delivery[1])))) {
            ...
            postLabel.setText("Ваша заявка отправлена на рассмотрение менеджеру!");
            postLabel.setVisible(true);
        }
    }
    ...
    void conditionClick(MouseEvent event) {
        ...
        countCreditBut.setVisible(true);
        timeCreditLAbel.setVisible(true);
        sumCreditField.setVisible(true);
        CreditChoiceBox.setVisible(true);
        SumCreditLabel.setVisible(true);
        postLabel.setText("Сумма заказа - " + fullCostOrder);
        postLabel.setVisible(true);
    }
    ...
    public void initialize(URL location, ResourceBundle resources) {
       ...
       countSpin.setEditable(true);
        ...
        ObservableList<String> langs = FXCollections.observableArrayList("3 мес.", "4 мес.", "5 мес.", "6 мес.", "7 мес.", "8 мес.", "9 мес.", "10 мес.", "11 мес.", "12 мес.", "18 мес.", "24 мес.");
        CreditChoiceBox.setItems(langs);
    }

...
public class ServerThread extends Thread {
    ...
    public void handle(JSONObject jsonQuery, PrintWriter writer) throws DAOException, SQLException {
        switch (DAOAction.valueOf(jsonQuery.get("action").toString())){
            ...
            case SEARCH_WORKER:{
                WorkerDAO workerDAO = DataSource.getInstance().getWorkerDAO();
                String search = jsonQuery.getString("param");
                String workers = gson.toJson(workerDAO.searchBySurname(search));
                JSONObject object = new JSONObject();
                object.put("workers", workers);
                System.out.println("SEARCH_WORKERS_BY_SURNAME");
                writer.println(object.toString());
                break;
            }
            ...
            case TOP_CUSTOMER:{
                OrderDAO orderDAO = DataSource.getInstance().getOrderDAO();
                Long begin = jsonQuery.getLong("beginDate")*1000;
                Long end = jsonQuery.getLong("endDate")*1000;
                String orders = gson.toJson(orderDAO.findSumCost(begin,end));
                JSONObject object = new JSONObject();
